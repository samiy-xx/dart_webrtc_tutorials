...
void main() {
  ...
  // Ice candidate from pc2, null if ice candidate generation has ended
  pc1.onIceCandidate.listen((RtcIceCandidateEvent e) {
    if (e.candidate != null)
      pc2.addIceCandidate(e.candidate);
  });

  // Ice candidate from pc1, null if ice candidate generation has ended
  ...
  if (MediaStream.supported) {
    window.navigator.getUserMedia(audio: true, video: true).then((LocalMediaStream stream) {
      ...
      // Fires onAddStream on pc2
      pc1.addStream(stream);

      // Fires onAddStream on pc1
